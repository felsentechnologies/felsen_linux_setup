...
## Criando a stack
cat > evolution_v2${1:+_$1}.yaml <<__FELSEN_MANAGED_FILE__
version: "3.7"
services:
...
## --------------------------- FELSEN --------------------------- ##
  ...
  evolution${1:+_$1}:
    image: atendai/evolution-api:v2.1.2 ## Versao da Evolution API
...
cat > dados_evolution_v2${1:+_$1} <<__FELSEN_MANAGED_FILE__
[ EVOLUTION API ]
