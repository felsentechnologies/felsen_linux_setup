...
while true; do
    ...
    echo -en "\e[33mDigite o dominio para o WebUI Ollama (ex: ollama.example.com): \e[0m" && read -r url_ollama
    echo ""

    ##Pergunta o Dominio para a ferramenta
    echo -e "\e[97mPasso$amarelo 2/2\e[0m"
    echo -en "\e[33mDigite o dominio para a API Ollama (ex: apiollama.example.com): \e[0m" && read -r url_apiollama
    echo ""
...
## Mensagem de Passo
echo -e "\e[97m- INICIANDO A INSTALACAO DO OLLAMA \e[33m[1/4]\e[0m"
echo ""
sleep 1
...
cat > ollama${1:+_$1}.yaml <<__FELSEN_MANAGED_FILE__
version: "3.7"
services:
...
if [ $? -eq 0 ]; then
    ...
    echo "Nao foi possivel criar a stack do Ollama"
fi
...
#    echo "Nao foi possivel Subir a stack do Ollama"
#fi
...
cat > dados_ollama${1:+_$1} <<__FELSEN_MANAGED_FILE__
[ OLLAMA ]
